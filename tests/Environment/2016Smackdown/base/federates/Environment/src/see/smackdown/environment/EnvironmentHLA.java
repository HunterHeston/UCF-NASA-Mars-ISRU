...
import see.smackdown.reference_frame.ReferenceFrame;
import see.smackdown.reference_frame.ReferenceFrameObject;
...
class EnvironmentHLA {
	...
	private static final String FEDERATION_NAME = "SEE 2016";
	private static final String FEDERATE_TYPE = "Environment";

	// Instance specific definitions.
	private String federate_name = "SEEEnvironment2016";
	...
	public EnvironmentHLA(String fed_name) {
	...
	public EnvironmentHLA(String fed_name, double lookahead_time) {
		// Set the federate name.
		federate_name = new String(fed_name);
	...
	public void set_host(String host_name) {
	 ...
	 * 
	 * This routine will return the wait status for federation time advancement.
	 * 
	...
	public void request_time_advance() {
		try {
			...
			System.out.println("\": Federate Not Execution Member! ");
	...
	// Environment federate HLA configuration.
	public void configure(String host_name, int port) {
		// Override the CRC host name.
		crc_host = new String(host_name);
	...
	public int initialize(boolean time_constrained_flag, boolean time_regulating_flag) {
		String rti_name;
		String rti_version;
		String hla_version;
		...
		try {
			...
			// Something went wrong in getting the ambassador or encoder
			// factory.
			System.out.print("Federate \"");
			System.out.print(federate_name);
		...
		if (rti_name.equals("pRTI 1516")) {
			local_settings_designator = new String("crcHost=" + crc_host + "\ncrcPort=" + crc_port);
		...
		// Connect to the RTI.
		try {
			System.out.println("CONNECTING to RTI:\"" + local_settings_designator + "\"");
			rti_ambassador.connect(env_fed_amb, CallbackModel.HLA_IMMEDIATE, local_settings_designator);
		...
		// Destroy the Federation Execution to cleanup an orphaned execution.
		try {
			System.out.println("CLEANUP Federation \"" + FEDERATION_NAME + "\"");
		...
		if (create_federation) {
			try {
				System.out.println("CREATING Federation \"" + FEDERATION_NAME + "\"");
				rti_ambassador.createFederationExecution(FEDERATION_NAME, FOM_MODULES, "HLAinteger64Time");
				...
				// NOTE: This isn't necessarily an error. However, we are going
				// to
		...
		try {
			System.out.println("JOINING Federation \"" + FEDERATION_NAME + "\"");
			rti_ambassador.joinFederationExecution(federate_name, FEDERATE_TYPE, FEDERATION_NAME, FOM_MODULES);
		...
		try {
			System.out.println("Attempting enable async");
			rti_ambassador.enableAsynchronousDelivery();
			System.out.println("Enabled Asnyc");
		} catch (RTIinternalError rti_error) {
			System.out.print("Federate \"");
			System.out.print(federate_name);
		...
		try {
			...
			if (enable_time_constrained) {
				...
				while (!env_fed_amb.is_time_constrained) {
					Thread.yield();
				}
			...
			if (enable_time_regulating) {
				...
				while (!env_fed_amb.is_time_regulating) {
					Thread.yield();
				}
	...
	public void add_published_frame(ReferenceFrame ref_frame) {
		env_fed_amb.add_published_frame(ref_frame, rti_ambassador);
	}

	// Add a expected reference frame.
	public void add_expected_frame(ReferenceFrame ref_frame, String name) {
	...
	public void shutdown() {
		...
		try {
			if (enable_time_constrained) {
				...
				System.out.println("Federate \"" + federate_name + "\": Disabled Time Constrained");
			}
		} catch (Exception e) {
			System.out.print("Federate \"");
			System.out.print(federate_name);
			System.out.print("\": Disable Time Constrained Error: ");
			System.out.println(e.getMessage());
		}
		try {
			if (enable_time_regulating) {
				rti_ambassador.disableTimeRegulation();
				System.out.println("Federate \"" + federate_name + "\": Disabled Time Regulating");
			}
		} catch (Exception e) {
			System.out.print("Federate \"");
			System.out.print(federate_name);
			System.out.print("\": Disable Time Regulating Error: ");
			System.out.println(e.getMessage());
		}
		...
		try {
			...
			System.out.println("Federate \"" + federate_name + "\": Resigned");
		} catch (Exception e) {
			System.out.print("Federate \"");
			System.out.print(federate_name);
			System.out.print("\": Resignation Error: ");
			System.out.println(e.getMessage());
		}
		...
		try {
			...
			System.out.println("Federate \"" + federate_name + "\": Destroyed Federation");
		} catch (Exception e) {
			System.out.print("Federate \"");
			System.out.print(federate_name);
			System.out.print("\": Federation Destruction Error: ");
			System.out.println(e.getMessage());
		}
		...
		try {
			...
			System.out.println("Federate \"" + federate_name + "\": Disconnected from RTI");
		} catch (Exception e) {
			System.out.print("Federate \"");
			System.out.print(federate_name);
			System.out.print("\": Disconnect Error: ");
			System.out.println(e.getMessage());
		}
	...
	public boolean wait_on_initialization_data() {
		...
		try {
			...
			// has been discovered.
			while (env_fed_amb.wait_on_init_discovery) {
				Thread.yield();
			}

		} catch (Exception e) {
			System.out.print("EnvironmentHLA \"");
			System.out.print(federate_name);
			System.out.println("\": Unexpected/unhandled error detected:");
			System.out.println(e.getMessage());
			return (false);
		...
		while (env_fed_amb.wait_on_init_data) {
			Thread.yield();
		}
	...
	private boolean recreate_federation() {
		// Destroy the Federation Execution.
		try {
			System.out.println("DESTROY Federation \"" + FEDERATION_NAME + "\"");
			rti_ambassador.destroyFederationExecution(FEDERATION_NAME);
			System.out.println("   DESTROYED");
		} catch (Exception e) {
			System.out.print("Federate \"");
		...
		try {
			System.out.println("CREATE Federation \"" + FEDERATION_NAME + "\"");
			rti_ambassador.createFederationExecution(FEDERATION_NAME, FOM_MODULES, "HLAinteger64Time");
			System.out.println("   CREATED");
		} catch (Exception e) {
			System.out.print("Federate \"");
	...
	public boolean is_execution_member() {
		boolean is_exec_member = true;
		try {
			rti_ambassador.getOrderName(OrderType.TIMESTAMP);
		} catch (FederateNotExecutionMember e) {
