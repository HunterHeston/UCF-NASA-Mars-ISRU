...
public class EarthMoonSystem {
	...
	public class Barycenter {
		...
		public Barycenter(String parent_name, String inertial_name, String rotating_name) {
	...
	public EarthMoonSystem(double mjd_utc) {
		...
		this.barycenter = new Barycenter("SolarSystemBarycentricInertial", "EarthMoonBarycentricInertial",
		...
		this.l2_frame = new ReferenceFrame("EarthMoonBarycentricInertial", "EarthMoonL2Rotating");
	}

	// JAT Time object constructor.
	public EarthMoonSystem(Time time) {
		// Instantiate the Earth-Moon barycentric reference frames.
		this.barycenter = new Barycenter("SolarSystemBarycentricInertial", "EarthMoonBarycentricInertial",
				"EarthMoonBarycentricRotating");
	...
	private void initialize_L2_frame() {
		...
		// Set up body mass ratio and mu ratios of the system; these are
		// constant
		// parameters that will enable calculation of position and velocity of
		// the managed frame with respect to the system barycenter (note that
		// the
		...
		// Set up the ratio of (L2 dist from body 1) / (body 2 dist from body
		// 1);
		...
		while ((d_x > 1e-15) || (d_x < -1e-15)) {
			func = x5_coeff * Math.pow(x_real, 5.0) + x4_coeff * Math.pow(x_real, 4.0)
					+ x3_coeff * Math.pow(x_real, 3.0) - x2_coeff * Math.pow(x_real, 2.0) - x1_coeff * x_real
					- x0_coeff;

			d_func = 5.0 * x5_coeff * Math.pow(x_real, 4.0) + 4.0 * x4_coeff * Math.pow(x_real, 3.0)
					+ 3.0 * x3_coeff * Math.pow(x_real, 2.0) - 2.0 * x2_coeff * x_real - x1_coeff;
		...
		// Build final overall constant to be used to scale position and
		// velocity
		// of body 2 (wrt system barycenter) to the pos/vel of the L2 point
		// (also
		...
		Vector3.scale(moon.inertial.position, body2_to_L2_scale_factor, l2_frame.position);
		Vector3.scale(moon.inertial.velocity, body2_to_L2_scale_factor, l2_frame.velocity);
	...
	private void update_barycenter_inertial(Time ephem_time, DE405 ephemeris) {
		...
		// NOTE: This will be in kilometers in the Solar System Barycenter
		// frame.
		em_pos_vel = ephemeris.get_planet_posvel(DE405_Body.EM_BARY, ephem_time);
	...
	private void update_L2_frame(Time ephem_time) {
		...
		Matrix3x3.copy(barycenter.rotating.T_parent_body, l2_frame.T_parent_body);
	...
	private void compute_rotating_frame(ReferenceFrame body_frame, ReferenceFrame rotating_frame) {
		...
		Vector3.cross(body_frame.position, body_frame.velocity, momentum_vector);
		...
		Vector3.transform(rotating_frame.T_parent_body, ang_rate_in_inertial, rotating_frame.attitude_rate);
