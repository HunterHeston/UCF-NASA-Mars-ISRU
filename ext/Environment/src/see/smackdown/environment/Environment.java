...
class Environment {
	...
	private static void process_preferences() {
		...
		federate_name = prefs.get("federate_name", "SEE Smackdown Environment");
		...
		// The default date is "4/19/2015 20:00:00 GMT" or TJD=17131.83333333334
		tjd_epoch = prefs.getDouble("TJD_epoch", 17131.83333333349);
	...
	private static class EnvOptionsParser extends CmdLineParser {
		...
		public static final Option DATE = new CmdLineParser.Option.StringOption('d', "date");

		// Set the starting execution date in UTC Julian date format.
		public static final Option JULIAN_DATE = new CmdLineParser.Option.DoubleOption('j', "JD");

		// Set the starting execution date in UTC modified Julian date format.
		public static final Option MJD = new CmdLineParser.Option.DoubleOption('m', "MJD");

		// Set the starting execution date in UTC truncated Julian date format.
		public static final Option TJD = new CmdLineParser.Option.DoubleOption('t', "TJD");
		...
		public static final Option DISABLE_HLA = new CmdLineParser.Option.BooleanOption('f', "hla");
	...
	private Environment(int year, int month, int day, int hour, int minute, double second) {
		// Set the current simulation epoch.
	...
	private void configure() {
		...
		this.DE405_data_dir = ephem_path.root_path + "data" + dir_sep + "core" + dir_sep + "ephemeris" + dir_sep
				+ "DE405data" + dir_sep;
		...
		while (continue_execution) {
			...
			if (enable_hla) {
				...
				while (env_hla.wait_for_time_advance()) {
					try {
						// Use a sleep here instead of a wait to play nice with
						// the CPU usage.
						Thread.sleep(10);
					...
					// Every two seconds check to see if our Federate is still
					// an execution
	...
	private void shutdown() {
		...
		// Unfortunately, we don't quite get the same seconds as we started
		// with.
	...
	private void initialize_environment() {
		...
		mars = new Planet(solar_system_barycenter.inertial, "Mars", DE405_Body.MARS);
...
//		try {
//			InteractionClassHandle ich = env_hla.rti_ambassador.getInteractionClassHandle("RoverInteraction");
//			env_hla.rti_ambassador.publishInteractionClass(ich);
//			ParameterHandleValueMap params;
//			params = env_hla.rti_ambassador
//					.getParameterHandleValueMapFactory()
//					.create(2);
//
//			ParameterHandle nameHandle =  env_hla.rti_ambassador.getParameterHandle(ich, "name");
//			ParameterHandle payloadHandle =  env_hla.rti_ambassador.getParameterHandle(ich, "payload");
//
//			params.put(nameHandle, "TestName".getBytes());
//			params.put(payloadHandle, "TestPayload".getBytes());
//
//			env_hla.rti_ambassador.sendInteraction(ich, params, "TEST".getBytes());
//			System.out.println("published interaction");
