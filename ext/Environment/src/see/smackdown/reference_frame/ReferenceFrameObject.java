...
public class ReferenceFrameObject {
	...
	public ReferenceFrameObject(ReferenceFrame frame, String name, EncoderFactory encoder_factory) {
	...
	public ReferenceFrameObject(ReferenceFrame frame, String name, ObjectInstanceHandle handle,
	...
	public void create_encoders(EncoderFactory encoder_factory) {
		...
		vec_encoder = encoder_factory.createHLAfixedArray(encoder_factory.createHLAfloat64LE(),
				encoder_factory.createHLAfloat64LE(), encoder_factory.createHLAfloat64LE());
		vec_dot_encoder = encoder_factory.createHLAfixedArray(encoder_factory.createHLAfloat64LE(),
				encoder_factory.createHLAfloat64LE(), encoder_factory.createHLAfloat64LE());
		q_vec_encoder = encoder_factory.createHLAfixedArray(encoder_factory.createHLAfloat64LE(),
				encoder_factory.createHLAfloat64LE(), encoder_factory.createHLAfloat64LE());
	...
	public void register_frame(RTIambassador rti_ambassador) {
		...
		try {
			...
			if (name_reserved) {
				instance_handle = rti_ambassador.registerObjectInstance(obj_class_handle, instance_name);
			}
	...
	public static void subscribe(RTIambassador rti_ambassador) {
		...
		try {
			rti_ambassador.subscribeObjectClassAttributes(obj_class_handle, attributeSet);
		} catch (Exception e) {
	...
	public static void publish(RTIambassador rti_ambassador) {
		...
		try {
			rti_ambassador.publishObjectClassAttributes(obj_class_handle, attributeSet);
		} catch (Exception e) {
	...
	@SuppressWarnings("unchecked")
	public AttributeHandleValueMap updateAttributes(RTIambassador rti_ambassador, LogicalTime logical_time) {
		AttributeHandleValueMap attribute_values = null;

		try {

			// Get an AttributeHandleValueMap from the RTI ambassador.
			attribute_values = rti_ambassador.getAttributeHandleValueMapFactory().create(5);
			...
			if (ref_frame.parent_frame_name != null) {
				...
				attribute_values.put(parent_frame_handle, name_encoder.toByteArray());
			}
			...
			//
			// Get references to embedded encoders.
			HLAfixedArray<HLAfloat64LE> hla_pos_vector = (HLAfixedArray<HLAfloat64LE>) (trans_state_encoder.get(0));
			HLAfixedArray<HLAfloat64LE> hla_vel_vector = (HLAfixedArray<HLAfloat64LE>) (trans_state_encoder.get(1));
			...
			attribute_values.put(trans_state_handle, trans_state_encoder.toByteArray());

			//
			// Encode the rotational state data.
			//
			// Get references to embedded encoders.
			HLAfixedRecord hla_quat = (HLAfixedRecord) (rot_state_encoder.get(0));
			HLAfloat64LE hla_q_scalar = (HLAfloat64LE) (hla_quat.get(0));
			HLAfixedArray<HLAfloat64LE> hla_q_vector = (HLAfixedArray<HLAfloat64LE>) (hla_quat.get(1));
			HLAfixedArray<HLAfloat64LE> hla_omega_vector = (HLAfixedArray<HLAfloat64LE>) (rot_state_encoder.get(1));
			...
			attribute_values.put(rot_state_handle, rot_state_encoder.toByteArray());
		...
		try {
			rti_ambassador.updateAttributeValues(this.instance_handle, attribute_values, null, logical_time);
