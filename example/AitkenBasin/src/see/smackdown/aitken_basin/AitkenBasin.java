...
package see.smackdown.aitken_basin;
...
class AitkenBasin {
	...
	private static void process_preferences() {
		...
		federate_name = prefs.get("aitken_basin_federate_name", "Aitken Basin Frame");
	...
	private static class ABFedOptionsParser extends CmdLineParser {
		...
		public static final Option HELP = new CmdLineParser.Option.BooleanOption('h', "help");
		...
		public static final Option RUN_TIME = new CmdLineParser.Option.IntegerOption('r', "run_time");

		// Set the name of this HLA federate.
		public static final Option FED_NAME = new CmdLineParser.Option.StringOption('n', "name");

		// Set the host name where the CRC/RTI is running.
		public static final Option CRC_HOST = new CmdLineParser.Option.StringOption("crc_host");

		// Set the port number to communicate with the CRC/RTI host.
		public static final Option CRC_PORT = new CmdLineParser.Option.IntegerOption("crc_port");
	...
	private AitkenBasin() {
		...
		em_bary_inrtl_frame = new ReferenceFrame("SolarSystemBarycentricInertial", "EarthMoonBarycentricInertial");
		em_bary_rot_frame = new ReferenceFrame("EarthMoonBarycentricInertial", "EarthMoonBarycentricRotating");
		earth_inrtl_frame = new ReferenceFrame("EarthMoonBarycentricInertial", "EarthCentricInertial");
		earth_pfix_frame = new ReferenceFrame("EarthCentricInertial", "EarthCentricFixed");
		moon_inrtl_frame = new ReferenceFrame("EarthMoonBarycentricInertial", "MoonCentricInertial");
		moon_pfix_frame = new ReferenceFrame("MoonCentricInertial", "MoonCentricFixed");
		em_L2_frame = new ReferenceFrame("EarthMoonBarycentricInertial", "EarthMoonL2Rotating");
		mars_inrtl_frame = new ReferenceFrame("SolarSystemBarycentricInertial", "MarsCentricInertial");
		mars_pfix_frame = new ReferenceFrame("MarsCentricInertial", "MarsCentricFixed");

		// Instantiate the new Aitken Basin local fixed frame.
		aitken_basin_frame = new ReferenceFrame("MoonCentricFixed", "AitkenBasinLocalFixed");

		// Inform the EnvironmentFederateAmbassador of expect reference frames.
		env_hla.add_expected_frame(sun_inrtl_frame, sun_inrtl_frame.ref_frame_name);
		env_hla.add_expected_frame(em_bary_inrtl_frame, em_bary_inrtl_frame.ref_frame_name);
		env_hla.add_expected_frame(em_bary_rot_frame, em_bary_rot_frame.ref_frame_name);
		env_hla.add_expected_frame(earth_inrtl_frame, earth_inrtl_frame.ref_frame_name);
		env_hla.add_expected_frame(earth_pfix_frame, earth_pfix_frame.ref_frame_name);
		env_hla.add_expected_frame(moon_inrtl_frame, moon_inrtl_frame.ref_frame_name);
		env_hla.add_expected_frame(moon_pfix_frame, moon_pfix_frame.ref_frame_name);
		env_hla.add_expected_frame(em_L2_frame, em_L2_frame.ref_frame_name);
		env_hla.add_expected_frame(mars_inrtl_frame, mars_inrtl_frame.ref_frame_name);
		env_hla.add_expected_frame(mars_pfix_frame, mars_pfix_frame.ref_frame_name);
	...
	private int initialize() {
		...
		if (env_hla.initialize(true, false) != 0) {
			return 1;
		}
	...
	private void run_loop() {
		...
		// data we'll be waiting on for initialization.
		...
		while (continue_execution) {
			...
			while (env_hla.wait_for_time_advance()) {
				// Use a sleep here instead of a wait to play nice with the CPU
				// usage.
				try {
					Thread.sleep(10);
				} catch (Exception e) {
				...
				// Every two seconds check to see if our Federate is still an
				// execution
				// member (i.e. has not lost a connection or has not been
				// resigned).
				if (env_hla.wait_for_time_advance() && ((++cnt % 200) == 0)) {
					cnt = 0;
					if (!env_hla.is_execution_member()) {
						System.out.print("AitkenBasin \"");
						System.out.print(federate_name);
						System.out.println("\": Federate is not Execution Member!");
			...
			if (exec_loop_counter > end_count) {
	...
	private void update_environment() {
		...
		aitken_basin_frame.time += 1.0 / (60.0 * 60.0 * 24.0);
	}
